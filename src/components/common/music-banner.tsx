/*
 * @Author: 成中锦
 * @Date: 2020-07-16 17:40:25
 * @LastEditTime: 2020-07-17 14:13:17
 * @FilePath: \myMusic\src\components\common\music-banner.tsx
 * @explain: 
 */ 
...
import "../../css/commonCom/music-banner.css"
...
const musicBanner:React.FC<{}> = memo( (props:any) => {
    const { list=[] , Active=0 , change=()=>{} , data:config={} }:bannerParams = props
    //= 控制动画的暂停和播放
    const [pause , changePause]:[string|boolean,any] = useState<string|boolean>( false )
    const [opacity , changeOpacity]:[number,any] = useState<number>( 1 )
    const [showPlay , changeShowPlay]:[number|boolean,any] = useState<number|boolean>( false )
    //= 当前轮播页
    const [nowActive , changeNowActive]:[number,any] = useState<number>( Active )
    //= banner的dom元素
    const banner:any = useRef<{current:null}>(null)
    //= 动画id
    const animate:any = useRef<{current:null}>(null)
    //= 判断是否是子元素
    const isChild = useCallback(( ref:{current?:any} , toTarget:Element):boolean => !ref.current || !toTarget ? false : toTarget.nodeType && ref.current.contains(toTarget),[]) 
    //= 鼠标事件
    const enter = useCallback((e:any):void => changePause(true) && e.stopPropagation(),[])
    const leave = useCallback((e:any):void => !isChild(banner , e.relatedTarget) && changePause(false),[banner,isChild])
    const showPlayBtn = useCallback((e:any,index:number):void => changeShowPlay(index),[])
    const hidePlayBtn = useCallback(e => changeShowPlay(false),[])
    
    const conf:bannerConf = useMemo(():bannerConf => {
    ...
    const lists:Array<any> = useMemo(():Array<any> => {
    ...
    const setUlStyle = useCallback(():bannerUlStyle => {
        ...
        const size = conf.page.size
        const obj:bannerUlStyle = { width:`${len * width}px` ,left:`${-size * width * (nowActive+1) }px` , transition: `${type} 0.5s linear`}
        if( type === "opacity" ) {
            obj.opacity = opacity
